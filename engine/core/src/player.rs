...
impl Player {
    ...
    pub fn rewind_root_movie(context: &mut UpdateContext<'_>) {
        if let Some(mc) = context
            .stage
            .root_clip()
            .and_then(|root| root.as_movie_clip())
    ...
    /// Get the SWF version this player is emulating.
    pub fn swf_version(&self) -> u8 {
        self.swf.version()
    }

    /// Get the emulated player version.
    pub fn player_version(&self) -> u8 {
        self.player_version
    }

    /// Whether the root movie uses ActionScript 3.
